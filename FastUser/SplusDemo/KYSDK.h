...
/**
 *  @method -(void)UPPayPluginResult:(NSString *)result
 *  支付宝本地客户端支付回调函数(调用快捷支付，按home键时，支付界面按钮回调)
 *  @param  result 支付结果
 */
//-(void)alipaysafeResult:(NSString *)result;
...
/**
 *  @method -(void)alipayCheckBack:(CHECK)result
 *  快捷支付返回结果验证结果
 */
//-(void)alipayCheckBack:(CHECK)result;
