...
-(void)splusStartPayClick:(id)sender
{
     ...
     *  @param ProductPrice       商品现在的价格，跟原价一样则相同 0.1  必填
     *  @param ProductOrignalPrice    商品原来的价格 0.1 必填
     *  @param ProductCount       购买商品的数量 必填
     */
    [[SplusInterfaceKit sharedInstance] splusQuotaPay:@"1" serverID:@"1111" serverName:@"湖南一区" RoleId:@"2222" RoleName:@"ak我杀猪" OutOrderId:@"外部订单号ID" Pext:@"扩展ext" ProductID:@"1" ProductPrice:@"0.1" ProductOrignalPrice:@"0.1" ProductCount:@"10"];
}
...
{
    [[SplusInterfaceKit sharedInstance] initSplus];//激活成功后，配置信息
    [[SplusInterfaceKit sharedInstance] splusScreenOrientation:UIInterfaceOrientationLandscapeRight];//设置SDK方向
}
...
-(void)SplusLoginOnSuccess:(SplusUser*)callbackUser
{
    ...
    NSLog(@"partner_Uid = %@", callbackUser.partner_uid);
